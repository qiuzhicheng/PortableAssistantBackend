...
import com.codejstudio.service.dto.UserDTO;
import com.codejstudio.service.util.Constant;
...
public class UserController {  
    ...
    @ResponseBody
    public ResponseJSON loginout(@RequestBody ResponseJSON rj,HttpSession session) throws ServletException, IOException{
		String  u_mobile = rj.getString("u_mobile");
		ResponseJSON resultRj=new ResponseJSON();
		if(session.getAttribute(Constant.CONSTANT_USER+u_mobile)==null || session.getAttribute(Constant.CONSTANT_USER+u_mobile).equals("") ){
			resultRj.put("status", 0);
		}
		else{
			session.removeAttribute(Constant.CONSTANT_USER+u_mobile);
			resultRj.put("status", 1);
		}
        return resultRj;
...
//        	session.setAttribute("user", user);
    ...
    public ResponseJSON random(@RequestBody JSONObject jo, HttpServletRequest req, HttpSession session){      
		...
		json.put(Constant.CONSTANT_VERIFICATIONCODE, str);
        session.setAttribute(Constant.CONSTANT_VERIFICATIONCODE+jo.getString("u_mobile"), str);
        System.out.println(session.getAttribute(Constant.CONSTANT_VERIFICATIONCODE+jo.getString("u_mobile")));
   ...
   @ResponseBody//在SpringMVC中可以在Controller的某个方法上加@ResponseBody注解，表示该方法的返回结果直接写入HTTP response body中。
    public ResponseJSON register(@RequestBody /*JSONObject*/ UserDTO user, HttpServletRequest req, HttpSession session){
        ...
        * 验证手机号是否已被注册
        */
       if(userService.ifselectUser(user.getU_mobile())==0){
    ...
    @ResponseBody//在SpringMVC中可以在Controller的某个方法上加@ResponseBody注解，表示该方法的返回结果直接写入HTTP response body中。
     public ResponseJSON register(@RequestBody JSONObject jo, HttpServletRequest req, HttpSession session){
         ...
         * 验证手机号是否已被注册
         */
        if(userService.ifselectUser(jo.getString("u_mobile"))==0){
	        String str=(String)session.getAttribute(Constant.CONSTANT_VERIFICATIONCODE+jo.getString("u_mobile"));
        	UserDTO user = new UserDTO();
	        if(str.equalsIgnoreCase(jo.getString(Constant.CONSTANT_VERIFICATIONCODE))){
	        	user.setU_mobile(jo.getString("u_mobile"));
	        	user.setU_username(jo.getString("u_username"));
	        	user.setU_password(jo.getString("u_password"));
	            user.setCreate_time(new Date());
	            user.setModify_time(new Date());
	            //System.out.println(jo.getString("mobile"));
	            //调用service
	            userService.register(user);
	            session.setAttribute(user.getU_mobile(), user);
	            json.put("u_mobile", user.getU_mobile());
	            json.put("u_username", user.getU_username());
	            json.put("status", 1);
	            session.removeAttribute(Constant.CONSTANT_VERIFICATIONCODE+user.getU_mobile());
...
//		UserDTO loginUser =(UserDTO) session.getAttribute(rj.getString("u_mobile"));
    	...
    	if(rj.getString("oldPassword").equals("123456"))
        {
...
//            	session.setAttribute("user", loginUser);
    ...
    @ResponseBody//在SpringMVC中可以在Controller的某个方法上加@ResponseBody注解，表示该方法的返回结果直接写入HTTP response body中。
    public ResponseJSON login(@RequestBody UserDTO loginUerDTO,  HttpSession session) throws ParamException{
        //用户的登录校验       
        ...
        if(dbUerDTO==null){//输入密码或者手机号有误
        	...
        	//将用户信息存入Session
        	session.setAttribute(Constant.CONSTANT_USER+dbUerDTO.getU_mobile(), dbUerDTO);
        	//返回给客户端登录信息
        	json.put("status", 1);
